...
// Vazias no build (ex.: CI sem secrets) → cai no fallback local.
const TURN_URLS =
  import.meta.env.VITE_TURN_URLS?.trim() ||
  'turn:192.168.18.130:3478?transport=udp,turn:192.168.18.130:3478?transport=tcp'
const ICE_SERVERS: RTCIceServer[] = [
  ...
  { urls: 'stun:stun.cloudflare.com:3478' },
  {
    urls: TURN_URLS.split(',')
      .map((u: string) => u.trim())
      .filter(Boolean),
    username: import.meta.env.VITE_TURN_USER?.trim() || 'cvturn',
    credential: import.meta.env.VITE_TURN_PASS?.trim() || 'cvpass2026'
  }
